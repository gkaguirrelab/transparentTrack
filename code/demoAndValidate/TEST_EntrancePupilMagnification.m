...
%
%   In Mathur 2013, empirical measurements of the ratio of pupil width to
%   height were made as a function of viewing angle:
%
...
% of these prior papers in which the camera maintained at a constant
...
myObjFunc = @(x) sum((mathurEq(azimuthsDeg) - localCompute(x,azimuthsDeg,pupilDiam,sceneGeometry,rayTraceFuncs)).^2);

x0=[0.5 -0.25];
[x, fVal]=fmincon(myObjFunc,x0,[],[],[],[],[-1 -1],[1 1]);

diamRatio = localCompute(x,azimuthsDeg,pupilDiam,sceneGeometry,rayTraceFuncs);

% This is Figure 10 of Mathur 2013
figure
...
%% LOCAL FUNCTION
function diamRatio = localCompute(x,azimuthsDeg,pupilDiam,sceneGeometry,rayTraceFuncs)
horizDiam=[];
vertDiam=[];
