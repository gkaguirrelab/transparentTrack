...
%% 2D ray tracing function through the cornea
...
rayTraceFuncs.cornea = {matlabFunction(outputRay2D)};


%% Distance of the ray intersection from the camera node
syms p1 p2 p3
syms theta_p1p2 theta_p1p3
outputRayEyeWorld2D_p1p2 = rayTraceFuncs.cornea{1}(p2,theta_p1p2,p1);
outputRayEyeWorld2D_p1p3 = rayTraceFuncs.cornea{1}(p3,theta_p1p3,p1);
...
rayTraceFuncs.cameraNodeDistanceError_p1p2 = {matlabFunction(...
    abs(cameraTranslationX-cameraPlaneX) + ...
    abs(cameraTranslationY-cameraPlaneY) ...
    )};
...
rayTraceFuncs.cameraNodeDistanceError_p1p3 = {matlabFunction(...
    abs(cameraTranslationX-cameraPlaneX) + ...
    abs(cameraTranslationY-cameraPlaneY) ...
    )};
...
outputRayEyeWorld_p1p2 = rayTraceFuncs.cornea{1}(p2, theta_p1p2, p1);
outputRayEyeWorld_p1p3 = rayTraceFuncs.cornea{1}(p3, theta_p1p3, p1);
...
rayTraceFuncs.virtualImageRay = {matlabFunction(outputRayEyeWorld)};
